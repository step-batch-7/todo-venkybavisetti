...
const parseNewTitle = function(keyValueTextPairs) {
  return keyValueTextPairs.split('&').reduce(pickDetails, {});
};

const getNewTitle = function(newTitle) {
  const { title } = parseNewTitle(newTitle);
  const newTodoTitle = {};
  newTodoTitle.title = putWhiteSpaces(title);
  return newTodoTitle;
};
...
const addNewTitle = function(req, res) {
  const todoList = loadTodoContent();
  const newTitle = getNewTitle(req.body);
