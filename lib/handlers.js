...
const serveHomePage = function(req, res) {
  ...
  const html = loadTemplate('index.html', {
    todoList: getTodoTitles(todoList)
  });
...
const createTitle = function(todo) {
  const html = `<div class= "todoBox"id="${todo.id}">
  ${todo.title}
  <div class = 'close' onclick = 'removeTodoTitle()' > <img src="https://img.icons8.com/color/48/000000/delete-forever.png" /></div>
  </div>`;
  return html;
};

const getTodoTitles = function(todoList) {
  return todoList.map(createTitle).join('\n');
};
...
const addNewTitle = function(req, res) {
  ...
  res.setHeader('Content-Type', CONTENT_TYPES.html);
  res.end(getTodoTitles(todoList));
};

const removeTodoBox = function(req, res) {
...
app.post('/removeTodoBox', removeTodoBox);
app.post('', notFound);
