...
const dataStoragePath = './data/todoList.json';
...
const storeData = function(todoList) {
  ...
  fs.writeFileSync(dataStoragePath, string, 'utf8');
};
...
const addNewTask = function(req, res) {
  ...
  const newTask = querystring.parse(req.body);
...
const addSubTasks = function(req, res) {
  ...
  const newTask = { text: subTask, done: false, id: generateSubTaskId++ };
  todoList[todoIndex].tasks.push(newTask);
  storeData(todoList);
  res.setHeader('Content-Type', CONTENT_TYPES.json);
