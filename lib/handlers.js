...
const createCookie = function(sessions, userName) {
...
const serveUserTodo = function(req, res) {
  ...
  if (checkPassword(userList, userName, password)) {
    const cookie = createCookie(sessions, userName);
    res.cookie('SID', cookie);
    res.redirect('/index.html');
  }
  const template = fillTemplate('login.html', 'Incorrect UserName/Password');
  res.send(template);
};
...
const logOut = function(req, res) {
  ...
  const cookieIndex = sessions.findIndex(session => session.SID === +cookie);
  sessions.splice(cookieIndex, 1);
  saveSession(sessions);
  res.clearCookie('SID');
  res.redirect('login.html');
};
